...
class UsersViewModel(settings: SettingsRepository) : ViewModel() {
    ...
    fun deleteUser(uid: String) { viewModelScope.launch { repo.deleteUser(uid); load() } }
    fun setPermissions(uid: String, perms: List<String>) { viewModelScope.launch { repo.setPermissions(uid, perms); load() } }
}
