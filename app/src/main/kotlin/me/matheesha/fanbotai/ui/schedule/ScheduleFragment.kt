...
class ScheduleFragment : Fragment() {
    ...
    private fun bindSchedule(s: TodaySchedule) {
        ...
        if (s.breaks.isEmpty()) {
            ...
            s.breaks.forEachIndexed { idx, brk ->
                addBreakRow(brk, idx)
            }
        }
    }
    ...
    private fun addBreakRow(brk: Break, idx: Int) {
        ...
        val rowView  = inflater.inflate(R.layout.item_break, binding.breaksContainer, false)
        rowView.findViewById<android.widget.TextView>(R.id.tvBreakTime).text =
            "${brk.start} – ${brk.end}"
        rowView.findViewById<android.widget.ImageButton>(R.id.btnDeleteBreak).setOnClickListener {
        ...
        binding.breaksContainer.addView(rowView)
    }

    private fun showAddBreakDialog() {
        if (todayDate.isEmpty()) return
        val dialogView = LayoutInflater.from(requireContext()).inflate(R.layout.dialog_break, null)
        val etStart = dialogView.findViewById<TextInputEditText>(R.id.etBreakStart)
        val etEnd   = dialogView.findViewById<TextInputEditText>(R.id.etBreakEnd)
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton("Add") { _, _ ->
                ...
                if (start.matches(Regex("\\d{2}:\\d{2}")) && end.matches(Regex("\\d{2}:\\d{2}"))) {
                    viewModel.addBreak(todayDate, start, end)
                } else {
                    Snackbar.make(binding.root, "Use HH:MM format", Snackbar.LENGTH_SHORT).show()
                }
            }
            .setNegativeButton("Cancel", null)
            .show()
